...
puts "Downloading #{mysql_tarball}"
...
puts "init_host = #{init_host}, my_ip = #{my_ip}, hosts = #{hosts}"
...
puts "wsrep_cluster_address = #{wsrep_cluster_address}"
...
puts "Joiner node sleeping 30 seconds to make sure donor node is up..."
sleep(node['xtra']['sleep'])
Chef::Log.info "Joiner node cluster address = gcomm://#{sync_host}:#{node['wsrep']['port']}"
puts "Joiner node cluster address = gcomm://#{sync_host}:#{node['wsrep']['port']}"
end
...
#execute "set-mysql-path" do
#  command "echo \"export PATH=$PATH:/usr/local/mysql/bin\" >> /etc/profile"
#end

execute "set-mysql-bin" do
  command "ln -s /usr/local/mysql/bin/mysql /usr/bin/mysql"
  action :run
  not_if { FileTest.exists?("/usr/bin/mysql") }
end
